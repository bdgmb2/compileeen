...
N_ASSIGN        : N_VARIABLE T_ASSIGN N_EXPR
               	  {
                	  ParseFunctions::printRule("N_ASSIGN", 
...
N_BLOCK         : N_VARDECPART N_PROCDECPART N_STMTPART
                  {
                	  ParseFunctions::printRule("N_BLOCK", 
...
N_COMPOUND      : T_BEGIN N_STMT N_STMTLST T_END
                  {
                  ParseFunctions::printRule("N_COMPOUND", 
                         "T_BEGIN N_STMT N_STMTLST T_END");
                  }
                ;
N_CONDITION     : T_IF N_EXPR T_THEN N_STMT
                  {
                  ParseFunctions::printRule("N_CONDITION", 
...
N_EXPR          : N_SIMPLEEXPR
			  ...
			  $$.type = $1.type; 
                ...
                | N_SIMPLEEXPR N_RELOP N_SIMPLEEXPR
                  {
              	  ParseFunctions::printRule("N_EXPR", 
                         "N_SIMPLEEXPR N_RELOP N_SIMPLEEXPR");
			  if ($1.type != $3.type) 
			  {
...
N_FACTOR        : N_SIGN N_VARIABLE
			  ...
			  $$.type = $1.type; 
                ...
                | T_LPAREN N_EXPR T_RPAREN
            	  {
             	  ParseFunctions::printRule("N_FACTOR", 
              	         "T_LPAREN N_EXPR T_RPAREN");
			  $$.type = $2.type; 
             	  $$.startIndex = $2.startIndex;
               	  $$.endIndex = $2.endIndex;
...
N_PROCHDR       : T_PROC T_IDENT T_SCOLON
			  ...
			  ParseFunctions::printSymbolTableAddition(lexeme, &info);
            	  bool success = ParseObj::scopeStack.top().addEntry
...
N_PROG          : N_PROGLBL T_IDENT T_SCOLON 
             	  {
             	  ParseFunctions::printRule("N_PROG",
...
N_READ          : T_READ T_LPAREN N_INPUTVAR N_INPUTLST T_RPAREN
              	  {
               	  ParseFunctions::printRule("N_READ",
...
N_SIMPLEEXPR    : N_TERM N_ADDOPLST
                	  {
                	  ParseFunctions::printRule("N_SIMPLEEXPR", 
               	         "N_TERM N_ADDOPLST");
			  $$.type = $1.type; 
               	  $$.startIndex = $1.startIndex;
                	  $$.endIndex = $1.endIndex;
...
N_VARDEC        : N_IDENT N_IDENTLST T_COLON N_TYPE 
              	  {
              	  ParseFunctions::printRule("N_VARDEC", 
             	         "N_IDENT N_IDENTLST T_COLON N_TYPE");
			  string varName = string($1);
			  ParseObj::variableNames.push_front(varName);
		
			  for (std::list<string>::iterator
             	       it=ParseObj::variableNames.begin();
			       it != ParseObj::variableNames.end(); it++) 
			  {
...
N_VARDECLST     : /* epsilon */
                ...
                | N_VARDEC T_SCOLON N_VARDECLST
              	  {
              	  ParseFunctions::printRule("N_VARDECLST", 
              	         "N_VARDEC T_SCOLON N_VARDECLST");
            	  }
                ;
N_VARDECPART    : /* epsilon */
                ...
                | T_VAR N_VARDEC T_SCOLON N_VARDECLST
               	  {
              	  ParseFunctions::printRule("N_VARDECPART",
                        "T_VAR N_VARDEC T_SCOLON N_VARDECLST");
               	  }
                ;
...
N_WHILE         : T_WHILE N_EXPR 
              	  {
               	  ParseFunctions::printRule("N_WHILE", 
...
N_WRITE         : T_WRITE T_LPAREN N_OUTPUT N_OUTPUTLST T_RPAREN
               	  {
                	  ParseFunctions::printRule("N_WRITE",
